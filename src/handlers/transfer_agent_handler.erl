...
i_order(#{card_from := CardFrom, card_to := CardTo,  amount := Amount}=Qs) ->
    ...
    case check_result_status(State1, Res2) of
        ...
        {error, _Status2} ->
            case State1 of
                <<"prepared">> -> send_rollback(#{host => Host1, port => Port1, transaction_order_id => TrOrder1});
                _ -> Res2
            end
    ...
    case State1 == <<"prepared">> andalso State2 == <<"prepared">> of
        true ->
            ...
            send_commit(#{host => Host2, port => Port2, transaction_order_id => TrOrder2});
        false ->
            case State1 of
                <<"prepared">> -> send_rollback(#{host => Host1, port => Port1, transaction_order_id => TrOrder1}), Res2;
                _ -> Res1
            end,
            case State2 of
                <<"prepared">> -> send_rollback(#{host => Host2, port => Port2, transaction_order_id => TrOrder2}), Res1;
                _ -> Res2
            end
    end.
...
send_rollback(MParams) ->
    Uri = ?FMT("http://~s:~s~s", [utl:to_list(maps:get(host, MParams)), utl:to_list(maps:get(port, MParams)), "/transaction/rollback"]),
    Params = maps:to_list(maps:with([transaction_order_id], MParams)),
    get_maps_by_request(maps:get(host, MParams), maps:get(port, MParams), <<"GET">>, Uri, ?OUTPUT_JSON_HEADERS, Params).
...
get_transaction_order_id(#{<<"result">> := Res}) ->
    {ok, maps:get(<<"transaction_order_id">>, Res)}.